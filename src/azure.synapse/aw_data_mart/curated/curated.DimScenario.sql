...
IF OBJECT_ID('[curated].[DimScenario]') IS NULL
BEGIN
CREATE TABLE [curated].[DimScenario]
(
...
CREATE OR ALTER PROCEDURE curated.curate_DimScenario
AS
BEGIN  
    SET NOCOUNT ON;
MERGE [curated].[DimScenario] AS T  
USING (
SELECT
    [ScenarioKey],
	[ScenarioName],
...
SET @max_surrogate_key = COALESCE((SELECT MAX() FROM [curated].[DimScenario]),0)

UPDATE A
SET A. = COALESCE(A., B.new_)
FROM [curated].[DimScenario] A
LEFT JOIN  (SELECT @max_surrogate_key + ROW_NUMBER() OVER (ORDER BY ) new_, natural_key_hash FROM [curated].[DimScenario]) B
ON  A.natural_key_hash = B.natural_key_hash 
END
GO

EXEC curated.curate_DimScenario
